...
namespace Dandelifeon {
    ...
    void workerTask(int id, Archive& archive, const Engine& engine) {
        ...
        auto init_layers = current_gen.toBitboards();
        SimulationResult best_res;
        engine.run(init_layers.first, init_layers.second, best_res, footprint);
        ...
        while (true) {
            ...
            for (int i = 0; i < mutation_count; ++i) {
                EvolutionManager::mutate(next_gen, rng, footprint);
            }

            auto next_layers = next_gen.toBitboards();
